...
export function HoldingsView({ symbol, pledged }: HoldingsViewProps) {
  ...
  if (loading) {
    return (
      <div className="flex flex-col space-y-6 animate-pulse">
        <div className="h-24 bg-[#1c2236] rounded-xl border border-[#2d3748]"></div>
        <div className="h-64 bg-[#1c2236] rounded-xl border border-[#2d3748]"></div>
      </div>
    );
  }
  ...
  if (error || !data) {
    return (
      <div className="p-8 text-center text-red-400 bg-[#1c2236] rounded-xl border border-[#2d3748]">
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Visual Breakdown */}
      <div className="bg-[#1c2236] p-6 rounded-xl border border-[#2d3748]">
        <h3 className="text-[#a0abc0] text-sm font-semibold mb-4 uppercase tracking-wider">Holding Pattern</h3>
        
        {!hasData ? (
          <div className="text-center text-slate-400 py-4">No aggregate holding data available.</div>
        ) : (
          <div className="space-y-4">
            ...
            <div className="h-4 w-full bg-[#111520] rounded-full overflow-hidden flex">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div className="bg-[#1c2236] p-4 rounded-xl border border-[#2d3748] flex flex-col justify-center items-center text-center">
          <div className="text-slate-400 text-xs uppercase mb-1">Shares Outstanding</div>
          <div className="text-lg font-bold text-white">{shares_outstanding ? formatNum(shares_outstanding) : 'N/A'}</div>
        </div>
        <div className="bg-[#1c2236] p-4 rounded-xl border border-[#2d3748] flex flex-col justify-center items-center text-center">
          <div className="text-slate-400 text-xs uppercase mb-1">Free Float</div>
          <div className="text-lg font-bold text-white">{float_shares ? formatNum(float_shares) : 'N/A'}</div>
        </div>
        <div className="bg-[#1c2236] p-4 rounded-xl border border-[#2d3748] flex flex-col justify-center items-center text-center">
          <div className="text-slate-400 text-xs uppercase mb-1">Promoter Holding</div>
          <div className="text-lg font-bold text-blue-400">{promoters_pct.toFixed(2)}%</div>
        </div>
        <div className="bg-[#1c2236] p-4 rounded-xl border border-[#2d3748] flex flex-col justify-center items-center text-center">
          <div className="text-slate-400 text-xs uppercase mb-1">Institutional</div>
          <div className="text-lg font-bold text-purple-400">{institutions_pct.toFixed(2)}%</div>
        </div>
      </div>

      {/* Insider Roster Table */}
      <div className="bg-[#1c2236] rounded-xl border border-[#2d3748] overflow-hidden">
        <h3 className="text-[#a0abc0] text-sm font-semibold p-4 border-b border-[#2d3748] bg-[#161b2b] uppercase tracking-wider">
        ...
        {(!data.roster || data.roster.length === 0) ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm text-slate-300">
              <thead className="text-xs text-slate-400 bg-[#161b2b] uppercase border-b border-[#2d3748]">
                <tr>
              ...
              <tbody>
                {data.roster.map((insider, i) => (
                  <tr 
                    key={i} 
                    className="border-b border-[#2d3748]/50 hover:bg-[#2d3748]/70 transition-colors cursor-pointer group"
                    onClick={() => setSelectedInsider(insider)}
                  >
                    <td className="px-4 py-3 font-medium text-white group-hover:text-indalpha-green transition-colors">{insider.name || 'N/A'}</td>
                    ...
                    <td className="px-4 py-3 text-right font-mono text-slate-200">
                      ...
                      {insider.pct > 0 && <span className="text-xs text-slate-500 ml-2">({insider.pct.toFixed(2)}%)</span>}
                    </td>
                    <td className="px-4 py-3">{insider.latest_transaction || '-'}</td>
                    <td className="px-4 py-3 text-slate-400">{insider.date || '-'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {selectedInsider && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={() => setSelectedInsider(null)}>
          <div 
            className="bg-[#1c2236] rounded-2xl border border-[#2d3748] shadow-2xl max-w-md w-full overflow-hidden flex flex-col transform transition-all"
          ...
          >
            ...
            <div className="p-6 space-y-6">
              <div>
                <div className="text-2xl font-bold text-white mb-1">{selectedInsider.name || 'Unknown Entity'}</div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-[#111520] p-4 rounded-xl border border-[#2d3748]/50">
                  <div className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Total Shares</div>
                  <div className="text-xl font-mono font-bold text-white">
                    {selectedInsider.shares > 0 ? selectedInsider.shares.toLocaleString() : 'N/A'}
                  </div>
                </div>
                <div className="bg-[#111520] p-4 rounded-xl border border-[#2d3748]/50">
                  <div className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Ownership</div>
              ...
              <div className="space-y-3">
                <h4 className="text-xs font-bold text-slate-500 uppercase tracking-wider border-b border-[#2d3748] pb-2">Recent Activity</h4>
                <div className="flex items-center justify-between bg-blue-900/10 border border-blue-900/30 p-3 rounded-lg">
                  <div className="flex items-center gap-3">
                    <Activity className={`w-5 h-5 ${selectedInsider.latest_transaction === 'Acquisition' ? 'text-indalpha-green' : selectedInsider.latest_transaction === 'Sale' ? 'text-indalpha-red' : 'text-slate-400'}`} />
                    <div>
                      <div className="text-sm font-bold text-white">{selectedInsider.latest_transaction || 'No recent transactions'}</div>
